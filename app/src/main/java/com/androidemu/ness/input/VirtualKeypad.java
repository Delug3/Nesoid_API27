package com.androidemu.ness.input;
...
import com.androidemu.ness.EmulatorActivity;
import com.androidemu.ness.R;
import com.androidemu.ness.wrapper.Wrapper;
...
public class VirtualKeypad {
	...
	public VirtualKeypad(View v, GameKeyListener l) {
		...
		settingsButton = createControl(R.drawable.setting_button);
	...
	public final void resize(int w, int h) {
		...
		settingsButton.hide(prefs.getBoolean("hideSettingsButton", false));
	...
	private void makeTopBottom(int w, int h) {
		dpad.move(0, (h - dpad.getHeight() - ( h / 13)));
		buttons.move(w - buttons.getWidth(), (h - buttons.getHeight() ) - ( h / 13 ));
		if (extraButtons.isEnabled()) {
			extraButtons.move(w - buttons.getWidth(), h - buttons.getHeight() * 7 / 3);
		}

		settingsButton.move( w - settingsButton.getWidth(), h / 2);
		int x = (w - buttons.getWidth() - selectStart.getWidth()) / 2;
		selectStart.move((w/2) - selectStart.getWidth() / 2 , h / 2);
	}
	...
	private int getControlStates(Control c, float x, float y, float size) {
		...
		if (c == settingsButton)
			return ((EmulatorActivity) context).openMenuItems();
