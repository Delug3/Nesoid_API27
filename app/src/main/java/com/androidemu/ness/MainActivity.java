...
public class MainActivity extends FileChooser {
	...
	public void beginCode() {
		...
		Uri uri =Uri.parse(getCacheDir()+"/ice_climber.nes");
	...
	public void assetFileToCache(){

		File f = new File(getCacheDir()+"/ice_climber.nes");
		if (!f.exists()) try {

			InputStream is = getAssets().open("ice_climber.nes");
